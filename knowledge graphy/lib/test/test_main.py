...
from entity.relation import EntityRelation
...
def test_main(data_dir):
    ...
    # 结果应为 (平行四边形, 是，闭合图形), (平行四边形，包括, 平行线)
    ...
    ner = NERecognition(dict_file=dict_file,pku_dict_file=None,seg_name='pkuseg', entity_aligment=align)
    erel = EntityRelation()

    em = EntityMaster(e_ner=ner, e_link=align, e_rel=erel, k_graphy=None)
