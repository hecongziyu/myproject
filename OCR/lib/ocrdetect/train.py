...
from config import *
from ocr_pos_dataset import GTDBDetection, detection_collate, GTDBAnnotationTransform
...
def train(args):
    ...
    valid_dataset = GTDBDetection(args, data_file=args.data_root,split='valid',
                            transform = GTDBTransform(data_root=args.root_path, window=args.window, size=args.size),
                            target_transform = GTDBAnnotationTransform())      
    ...
    # 
    criterion = MultiBoxLoss(args, cfg, overlap_thresh=args.pos_thresh, bkg_label=0, neg_pos=3)
    loc_loss = 0
    conf_loss = 0    
    epoch = 0
    ...
    for iteration in range(args.start_iter, cfg['max_iter']):
        ...
        # print('loss l:', loss_l, 'loss c:', loss_c)
        ...
        if iteration!=0 and (iteration % epoch_size == 0):
            ...
            validation_loss = validate(args, ssd_net, criterion, cfg, valid_dataset, valid_data_loader)
...
def validate(args, net, criterion, cfg, dataset, valid_data_loader):
    try:
