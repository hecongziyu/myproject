...
from qr_lmdb_dataset import QRDataset,AnnotationTransform
...
def train(args):
    ...
    dataset = QRDataset(data_dir=args.dataset_root,
                             window=args.window,
                             transform=QRTransform(window=args.window, max_width=1024, size=cfg['min_dim']),
...
def save_model(args, cfg, net, filename):
		torch.save(net.state_dict(),
					os.path.join(args.root_path,'ckpts', 'qr_best_ssd' + str(args.model_type)  + '.pth'))	
...
if __name__ == '__main__':
    ...
    try:
        ...
        args.resume = join(args.root_path, 'ckpts', 'qr_best_ssd300.pth')   
