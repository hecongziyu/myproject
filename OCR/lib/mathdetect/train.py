...
def train(args):
    ...
    dataset = FormulaDataset(data_dir=args.dataset_root,
                             window=args.window,
                             transform=FormulaTransform(window=args.window, max_width=1024, size=cfg['min_dim']),
...
def save_model(args, cfg, net, filename):
		torch.save(net.state_dict(),
					os.path.join(args.root_path,'ckpts',
                                   'weights_' + args.exp_name, args.detect_type + 'best_ssd' + str(args.model_type)  + '.pth'))	
