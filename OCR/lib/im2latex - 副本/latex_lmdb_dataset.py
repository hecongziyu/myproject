...
if __name__ == '__main__':
    ...
    # random_sel = np.random.randint(len(dataset) - len(dataset) , len(dataset), 5).tolist()
    # print('random_sel:', random_sel)
    random_sel = [2595,2595,2595,2595,2595,2595,2595]
    for idx in random_sel:
        image, target = dataset[idx]
        image = image.astype(np.uint8)
        print(idx , '--->', ' image shape:', image.shape)
        # cv2.imwrite(os.path.sep.join([args.data_root,'valid_img',f'{idx}.png']),image)
