...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb > 0:
            ...
            giou = bbox_iou(pbox.t().float(), tbox[i].float(), x1y1x2y2=False, GIoU=True)  # giou(prediction, target)
            ...
            # tobj[b, a, gj, gi] = 1
            tobj[0,1,10,10] = 1
            # print(' b: %s, a:%s, gj:%s, gi:%s obj:%s' %(b,a,gj,gi,tobj[b, a, gj, gi]))
            # Class
...
def build_targets(p, targets, model):
    ...
    nt = targets.shape[0]
    # print('targets size:', targets, ' nt :', nt)
    # print('predict size :',len(p), ':' ,p[0].size(), ':', p[1].size())
    ...
    for i, j in enumerate(model.yolo_layers):
        ...
        # print('gain :', gain, ' p[i].shape : %s ， %s' % (i, p[i].shape))
        ...
        if nt:
            ...
            # 面积大小比较   ？？？？？
            # print('比较面积 \n anchors %s  \n targets %s  \n after gain %s' % (anchors,targets,t) )
            j = wh_iou(anchors, t[:, 4:6]) >  model.hyp['iou_t'] # iou(3,n) = wh_iou(anchors(3,2), gwh(n,2))
            # print('比较面积 结果：', j, ' iou t :', model.hyp['iou_t'])
        ...
        gwh = t[:, 4:6].float()  # grid wh
        gij = (gxy - offsets).float()
        gi, gj = gij.T  # grid xy indices

        # Append
        indices.append((b, a, gj.long(), gi.long()))  # image, anchor, grid indices
